...
export async function query(sql: string, params?: any[]) {
  ...
  if (s.startsWith('insert into patients')) {
    const id = uuidv4();
    const patient = { id, patient_code: `P-${id.slice(0,8)}`, full_name: params?.[0] || 'Unknown', contact: params?.[1] || '', vehicle_plate: params?.[2] || '', arrival_time: now(), current_stage: 'Gate', last_updated: now() };
  ...
  if (s.startsWith('insert into finances')) {
    ...
    finances.push(row);
    audit_logs.push({ id: uuidv4(), user_id: null, action: 'Generate bill', module: 'Finance', details: `Finance completed for patient ${row.patient_id}`, created_at: now() });
    return { rows: [row] };
  }
